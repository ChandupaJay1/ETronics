async function loadData() {
    ...
    if (response.ok) {
        ...
        //load color list
        loadOption("pick-color", json.colorList, "name");
        //load color list
...
function loadOption(prefix, dataList, property) {
    ...
    dataList.forEach(data => {
        ...
        li_clone.addEventListener("click", e => {
            
            options.querySelectorAll("#"+prefix +"-a").forEach( item => {
                item.classList.remove("chosen");
            });
            
            li_clone.querySelector("#"+prefix +"-a").classList.add("chosen");
            
        });

        if (prefix === "pick-color") {
            console.log(data[property]);
            li_clone.classList.add(data[property]);
...
async function searchProducts(firstResult) {
    ...
    let category_name = document.getElementById("category-options")
            .querySelector(".chosen")?.innerHTML;

    let color_name = document.getElementById("pick-color-options")
            .querySelector(".chosen")?.style.backgroundColor;
...
function updateProductView(json) {
    ...
    json.productList.forEach(product => {
        ...
        st_product_clone.querySelector("#st-product-a").href = "product-details.html?id=" + product.id;
